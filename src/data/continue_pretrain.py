...
def _preprocess_function(tokenizer,examples):
    str_prompts = examples["text"]

    tokenized_prompts = tokenizer(str_prompts,padding="do_not_pad",truncation=True,max_length=512)
    
    input_ids = [
        torch.tensor(s + [tokenizer.eos_token_id]).long() for s in tokenized_prompts['input_ids']]
    labels = [
        torch.tensor(s + [tokenizer.eos_token_id]).long() for s in tokenized_prompts['input_ids']]
...
def make_ct_data_module(data_args,model_args,tokenizer):
